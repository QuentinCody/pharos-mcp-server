...
// src/index.ts
var PharosMCP = class extends McpAgent {
  constructor() {
    super(...arguments);
    ...
    this.PHAROS_GRAPHQL_ENDPOINT = "https://pharos-api.ncats.io/graphql";
  }
  static {
    __name(this, "PharosMCP");
  }
  async init() {
    console.error("Pharos MCP Server initialized.");
    this.server.tool(
      "pharos_graphql_query",
      `Executes a GraphQL query against the Pharos API (https://pharos-api.ncats.io/graphql). Pharos provides comprehensive information on biological targets (proteins), diseases, and ligands (small molecules/drugs). Query for specific entities and their relationships. For example, to find information about a target by UniProt ID: '{ target(q: { uniprot: "P05067" }) { name tdl description pathways { name type } } }'. To find information about a disease: '{ disease(name: "Alzheimer Disease") { name description targets(top: 3) { name preferredSymbol tdl } } }'. To find information about a ligand (e.g., by ChEMBL ID, which can be used as ligid): '{ ligand(ligid: "CHEMBL12") { name smiles isdrug activities(all: true, top: 2) { type value target { name } } } }'. Use GraphQL introspection for schema discovery: '{ __schema { queryType { name } types { name kind description fields { name args { name type { name ofType { name } } } } } } }'. Refer to the Pharos GraphQL API documentation (schema provided) for more examples and details. If a query fails, check the syntax and retry with introspection.`,
      {
        query: z.string().describe(
          `The GraphQL query string to execute against the Pharos GraphQL API. Example: '{ target(q: { uniprot: "P05067" }) { name tdl } }'. Use introspection queries like '{ __schema { queryType { name } types { name kind } } }' to discover the schema. `
        ),
  ...
  async executePharosGraphQLQuery(query, variables) {
    try {
      const headers = {
      ...
      console.error(`Client-side error during Pharos GraphQL request: ${error3 instanceof Error ? error3.message : String(error3)}`);
...
var src_default = {
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname === "/sse" || url.pathname.startsWith("/sse/")) {
      return PharosMCP.serveSSE("/sse").fetch(request, env2, ctx);
    }
...
// .wrangler/tmp/bundle-wKBRIv/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-wKBRIv/middleware-loader.entry.ts
